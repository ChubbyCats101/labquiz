...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${API_BASE}/teacher`, {
      headers: {
    ...
    const data = text ? (JSON.parse(text) as TeacherSuccess | TeacherError) : null;
    ...
    return NextResponse.json({ error: "Unable to reach teacher service" }, { status: 502 });
  }
}
